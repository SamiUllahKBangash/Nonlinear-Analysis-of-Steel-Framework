...
while lamda<=lamdamax
    ...
    while tau_repeat_corrector
        ...
        %-----FINT-PREDICTOR TRANSFORMATION AND PROJECTION-----------------
        for iel=1:Nel
            elnodes=elems(iel,1:2);
            nodexy=Nodes_updated(elnodes,:);
            E1=nodexy(2,:)-nodexy(1,:);
            ...
            Pi_trial=fint_predictor(1,iel);Mi_trial=fint_predictor(3,iel);Pj_trial=fint_predictor(4,iel);Mj_trial=fint_predictor(6,iel);
            pi_trial=Pi_trial/Py; mi_trial=Mi_trial/Mp; pj_trial=Pj_trial/Py; mj_trial=Mj_trial/Mp;
            ...
            if point_projection_radial
                for x=1:2
                    if abs(phi((3*x)-1,iel)-1)<=tol && phi((3*x),iel)<=(1+tol) && (phi((3*x)-2,iel)>(1+tol) || phi((3*x)-2,iel)<(1-tol))
                        ...
                        if flag==1
                            fint_predictor([3*x-2 3*x],iel)=correc*fint_predictor([3*x-2 3*x],iel);
                            ...
                            fint_predictor([3*x-2 3*x],iel)=fint_predictor([3*x-2 3*x],iel)+[correc(1)*Py;correc(2)*Mp];
            ...
            elseif point_projection_normal
                for x=1:2
                    if abs(phi((3*x)-1,iel)-1)<=tol && phi((3*x),iel)<=(1+tol) && (phi((3*x)-2,iel)>(1+tol) || phi((3*x)-2,iel)<(1-tol))
                        ...
                        fint_predictor([3*x-2 3*x],iel)=[psol*Py;msol*Mp];
        ...
        for iel=1:Nel
            ...
            [kt_G,kt_L,ke_L,kg_L,km_L,T,G]=kelemp_2ndorder_pred_II(nodexy,E,I,A,Py,Mp,Forces_internal_prev,Forces_internal_midstep,tol);
