%Welcome to Geometric-Inelastic_Euler_Simple Routine. This routine is intended
%to run a Standard Second Order Inelastic Analysis for 2D frame
%Structures. Simple Step Euler will be employed with standard updated
%Lagrangian Approach.
...
dlamda_1=25                         %incremental load ratio/step. |dlamda_1| must be < than lamdamax
...
while lamda<=lamdamax
    ...
    for iel=1:Nel
        ...
        [kt_G,kt_L,ke_L,kg_L,km_L,T,G]=kelemp_secondorder(nodexy,E,I,A,Py,Mp,Pi,Mi,Pj,Mj,tol); %kt=globalized tangent stiffness matrix (Ziemian eq 8.5)
    ...
    while tau_repeat                   %tau repeat loop for iteratively determining second order tau
        ...
        nodes_temp=Nodes_updated;      %array to store updated configuration after load step
        for inode=1:Nnodes
        ...
        for iel=1:Nel
            ...
            nodexy=nodes_temp(elnodes,:); %updated geometry at the end of load step
        ...
        if find(tau_array)
            ...
            for iel=1:Nel
                ...
                elnodes=elems(iel,1:2);
                nodexy=Nodes_updated(elnodes,:); %coordinates prior to load step.
                E1=nodexy(2,:)-nodexy(1,:);
                L=norm(E1); E1=E1/L;
                Qrot=[E1(1) E1(2) 0;
                    -E1(2) E1(1) 0;
                ...
                fint(:,iel)=T2*T'*fint(:,iel);   %Transforming fint to updated Ref. Configuration
            ...
            tau_repeat=false;  %flag to deactivate tau_repeat loop and march system forward
        end
    end
...
memberno=3;   %to be specified by the Analyst for plotting purposes
