function[kt_G,kt_L,ke_L,kg_L,km_L,T,G]=kelemp_2ndorder_pred_II(nodexy,E,I,A,Py,Mp,Forces_internal_prev,Forces_internal_midstep,tol)
...
if abs(phi_i_prev -1)<=tol
    dphipi=(2*pi_mid/Py)   +   (7*pi_mid*mi_mid^2/Py);
    dphimi=2*mi_mid/Mp   +   (7*pi_mid^2*mi_mid/Mp);
end

if abs(phi_j_prev -1)<=tol    
    dphipj=(2*pj_mid/Py)   +   (7*pj_mid*mj_mid^2/Py);
    dphimj=(2*mj_mid/Mp)   +   (7*pj_mid^2*mj_mid/Mp);
end
